...
class LGBMClassifierOptimizer:
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        model = LightGBMClassifierModel(
            ...
            early_stopping_rounds=self.early_stopping_rounds,
            val_fraction=self.val_fraction,
            scale_pos_weight=trial.suggest_float("scale_pos_weight", 0.1, 4.0, log=True),
            learning_rate=trial.suggest_float("learning_rate", 0.005, 0.1, log=True),
...
class OrdinalXGBoostClassifierOptimizer:
    """Bayesian hyperparameter search for OrdinalClassifierModel(XGBoost), maximising macro-F1.

    Each trial fits K-1 binary classifiers, so early stopping is disabled on
    the inner models to avoid triple-nested splits.
    """

...
THRESHOLD_S = -120  # 2 minutes early
SRC = "data/dataset_with_weather.parquet"
TMP = "data/dataset_with_weather.parquet.tmp"

print(f"Dropping rows with arrival_delay_s or departure_delay_s < {THRESHOLD_S}s ({abs(THRESHOLD_S) // 60} min early)...")
...
try:
    for i, batch in enumerate(pf.iter_batches()):
        ...
        filtered = table.filter(
            pc.and_(
                pc.greater_equal(table["arrival_delay_s"], THRESHOLD_S),
                pc.greater_equal(table["departure_delay_s"], THRESHOLD_S),
            )
        )
